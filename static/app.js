 ...
 * the License.
 */
var serverPath = '//ibde3app.appspot.com/';
...
function updateStateUi(state) {
	...
	var essaiElement = document.getElementById('essai');
	var stateCount = state['count'];
	var essai = state['essai'];
	// var user = JSON.parse(state['userClick']);
	var user = state['userClick'];
	...
	if (!stateCount) {
		...
		//console.log('EGAUX!!!');
		//console.log('id = ' + id.toString() + ' et user= ' + user.toString());
		var name = gapi.hangout.getParticipantById(user.toString());
		name = name.person.displayName;
		setText(countElement, stateCount.toString());
		if (id.toString() == user.toString())
			setText(essaiElement, name.toString());
	}
}
...
function oneParti() {
	console.log('Un seul participant!');
}

function unNouveauParti(nbr) {
	console.log(nbr + ' participants');
}
	...
	var apiReady = function(eventObj) {
		if (eventObj.isApiReady) {
			...
			gapi.hangout.data.onStateChanged.add(function(eventObj) {
				updateStateUi(eventObj.state, eventObj.person);
			});
			gapi.hangout.onParticipantsChanged.add(function(eventObj) {
				var par = eventObj.participants.length.toString();
				if (par == 1) {
					oneParti();
				} else {
					unNouveauParti(par);
